...
<template>
  <transition name="fade">
    <div v-if="isOpen" class="z-50 fixed bottom-0 bg-indigo-500 text-white">
      <div class="p-4">
        <div class="flex justify-center items-center space-x-2">
          <InfoIcon class="h-12 w-12" />
          <div class="flex-grow">
            <div>
          ...
          <div class="flex-shrink flex justify-end">
            <button
              class="py-1 px-3 rounded bg-gray-900 uppercase hover:bg-gray-800"
              @click="accept"
            >
              {{ buttonTextAccept }}
            </button>
            <button
              class="py-1 px-3 uppercase hover:text-indigo-100"
              @click="deny"
            >
              {{ buttonTextDeny }}
            </button>
